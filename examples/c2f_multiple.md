...
Finally, we apply this process to the HMM example to do inference over multiple time steps simultaneously. If we're doing inference over a 10 step hmm, for example, it may be more efficient to pair up each state, do inference over this set of 5 coarsened states, then use the likelihoods of these joint states to guide their individuation.
